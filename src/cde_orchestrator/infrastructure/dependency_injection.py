...
class DIContainer:
    def __init__(self, state_file_path: str, workflow_file_path: str, recipe_dir: str):
        ...
        state_path = Path(state_file_path) if isinstance(state_file_path, str) else state_file_path
        workflow_path = Path(workflow_file_path) if isinstance(workflow_file_path, str) else workflow_file_path
        ...
        # Use Cases
        self.manage_state_use_case = ManageStateUseCase(state_store=self.state_repository)
        ...
        self.select_workflow_use_case = SelectWorkflowUseCase(workflow_patterns=workflow_patterns)
    ...
    def _get_workflow_repository(self) -> YAMLWorkflowRepository:
        ...
        if self._workflow_repository is None:
            if not self._workflow_path.exists():
                logger.warning(f"Workflow file not found at {self._workflow_path}, creating default")
            ...
            self._workflow_repository = YAMLWorkflowRepository(workflow_path=self._workflow_path)
