...
SCENARIOS = [
    ...
    (
        "Investigate the best Python libraries for asynchronous programming in 2025",
        AgentType.DEEPAGENTS,
    ),
    ("Create a prototype for a new feature that uses websockets", AgentType.DEEPAGENTS),
    (
        "Refactor the entire authentication module to improve security",
        AgentType.DEEPAGENTS,
    ),
    ...
    (
        "What is the cyclomatic complexity of the `calculate_metrics` function?",
        AgentType.CODEX,
    ),
    ...
    (
        "Close the loop on task TICKET-789 by committing the changes and updating the status.",
        AgentType.ROVODEV,
    ),
    # Gemini Scenarios (Code Completion, Light Research, Docs)
    (
        "Help me finish this function to calculate the average of a list",
        AgentType.GEMINI,
    ),
...
def test_agent_selection_scenarios(task_description, expected_agent):
    ...
    assert (
        suggested_agent == expected_agent
