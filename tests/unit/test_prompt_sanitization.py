...
@pytest.mark.asyncio
async def test_prompt_manager_sanitizes_context(tmp_path):
    ...
    result = await manager.load_and_prepare(
        poml_file,
...
@pytest.mark.asyncio
async def test_missing_context_key_raises(tmp_path):
    poml_file = tmp_path / "missing.poml"
    poml_file.write_text("Value: {{USER_PROMPT}}", encoding="utf-8")

    manager = PromptAdapter(allowed_placeholders={"USER_PROMPT"})
    with pytest.raises(PromptValidationError):
        await manager.load_and_prepare(poml_file, {})


@pytest.mark.asyncio
async def test_disallowed_placeholder_rejected(tmp_path):
    poml_file = tmp_path / "disallowed.poml"
    poml_file.write_text("Value: {{UNSAFE}}", encoding="utf-8")

    manager = PromptAdapter(allowed_placeholders={"USER_PROMPT"})
    with pytest.raises(PromptValidationError):
        await manager.load_and_prepare(poml_file, {"UNSAFE": "value"})
